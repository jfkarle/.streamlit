        ...
        if st.sidebar.button("Find Alternatives", key="find_relaxed"):
            if st.session_state.current_job_request:
                ...
                slots, message, _, was_forced = ecm.find_available_job_slots(
                    **req, force_preferred_truck=(not relax_truck_input), relax_ramp_constraint=relax_ramp_input)
                st.session_state.info_message, st.session_state.found_slots = message, slots
                st.session_state.selected_slot, st.session_state.was_forced_search = None, was_forced
...
if st.session_state.found_slots and not st.session_state.selected_slot:
    ...
    # (Your two-tiered display logic can go here once the app is stable)
    cols = st.columns(3)
    for i, slot in enumerate(st.session_state.found_slots):
        with cols[i % 3]:
            with st.container(border=True):
                ...
                st.markdown(f"**Time:** {ecm.format_time_for_display(slot.get('time'))}")
                st.markdown(f"**Truck:** {slot.get('truck_id', 'N/A')}")
                ramp_details = ecm.get_ramp_details(slot.get('ramp_id'))
                st.markdown(f"**Ramp:** {ramp_details.ramp_name if ramp_details else 'N/A'}")
