...
def generate_daily_planner_pdf(report_date, jobs_for_day):
    ...
    # --- Step 1: Get Tide Data from the first scheduled job ---
    ...
    # --- Header & Column Drawing (Your Original Formatting) ---
    ...
    for hour in range(start_hour, end_hour + 1):
        for minute in [0, 15, 30, 45]:
            ...
            # Set text color based on tide
            text_color = colors.black
            if is_tide_time(high_tides): text_color = colors.darkorange
            elif is_tide_time(low_tides): text_color = colors.red
            ...
            c.setFillColor(colors.black) # Reset color for all other drawing
    ...
    # --- FIX #2: Draw the Bottom Border LAST to ensure it is visible ---
    c.setLineWidth(1.0)
    c.line(margin, bottom_y, width - margin, bottom_y)
        
    # --- FIX #1: Draw Tide Footnote ---
